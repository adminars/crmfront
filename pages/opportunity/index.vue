<template>
  <v-layout>
    ...
    <v-flex>
      <v-data-table :headers="headers" :items="list.data" item-key="name" class="elevation-1">
        <template v-slot:items="props">
          ...
          <td class="text-xs-right">
            {{ props.item.city }}
            <br />
            {{ props.item.familyname + ' ' + props.item.familyname }}
            <br />
            ({{ props.item.phone1 }})
          </td>
          ...
          <td
            class="text-xs-right"
          >{{ props.item.account_detail != null ? props.item.account_detail.store_name : ''}}</td>
          <td
            class="text-xs-right"
          >{{ props.item.user_detail != null ? (props.item.user_detail.familyname ) : '' }}</td>
          <td
            class="text-xs-right"
          >{{ props.item.owner_detail != null ? (props.item.owner_detail.familyname ) : props.item.user_detail != null ? (props.item.user_detail.familyname ) : '' }}</td>
